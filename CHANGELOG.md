...
- `CVE-2026-44405` 보완 통제로 호스트 키 사전점검과 인증 연결 모두에서
  RSA/SHA-1 서명 방식인 `ssh-rsa`를 비활성화하고 두 경로의 정책을 단일 함수로 고정
- RSA/SHA-1 전용 루프백 서버가 자격증명을 받기 전에
  `SSH_ALGORITHM_INCOMPATIBLE`로 차단되는 회귀 테스트 추가
